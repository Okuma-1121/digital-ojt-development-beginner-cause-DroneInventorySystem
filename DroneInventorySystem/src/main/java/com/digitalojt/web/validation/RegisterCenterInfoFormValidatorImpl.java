...
// すべてのフィールドが空かをチェック　　→不要　画面の機能で必須項目の入力をチェックしているため
		...
		if (form.getMaxStorageCapacity() != null) {
		    if (form.getMaxStorageCapacity() < 0 || form.getMaxStorageCapacity() > SearchParamsLimits.MAX_STORAGE_CAPACITY) {
		...
		if (form.getCurrentStorageCapacity() != null) {
		    if (form.getCurrentStorageCapacity() < 0 || form.getCurrentStorageCapacity() > form.getMaxStorageCapacity()) {
